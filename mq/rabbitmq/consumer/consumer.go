...
var (
	...
	exchange          = flag.String("exchange", "tt-exchange", "Durable, non-auto-deleted AMQP exchange name")
	exchangeType      = flag.String("exchange-type", "topic", "Exchange type - direct|fanout|topic|x-custom")
	queueName         = flag.String("queue", "tt-queue", "Ephemeral AMQP queue name")
	bindingKey        = flag.String("key", "black", "AMQP binding key")
	consumerTag       = flag.String("consumer-tag", "black-consumer", "AMQP consumer tag (should not be blank)")
	lifetime          = flag.Duration("lifetime", 15*time.Minute, "lifetime of process before shutdown (0s=infinite)")
...
func main() {
	c, err := NewConsumer(*uri, *exchange, *exchangeType, *queueName, *bindingKey, *consumerTag)
	if err != nil {
		ErrLog.Fatalf("%s", err)
	}
